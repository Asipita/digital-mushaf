...
export function Chapter({ surah }: { surah: Chapter }) {
  ...
  return (
    <div className="border rounded flex items-center text-gray-600 transition-all duration-300 hover:cursor-pointer hover:shadow-sm hover:border-slate-600 group">
      <span className="px-2 basis-8 border-r group-hover:border-r-slate-600 h-full flex items-center">
        {id}
      </span>
      ...
      <div className="flex flex-col gap-1 ml-auto py-2 px-2 border-l w-1/5 group-hover:border-l-slate-600">
        <span className="text-sm font-semibold text-right">{name_arabic}</span>
